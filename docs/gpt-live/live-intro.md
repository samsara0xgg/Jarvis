...
Source: https://developers.openai.com/api/docs/guides/live
...
Note: this page's rendered content did not contain any JSON payload examples or literal event names beyond `session.started`; other event/field names referenced by name are on the sibling pages (live-prompting, live-delegation, live-conversations, voice-webrtc, voice-websockets, voice-server-controls, voice-sip).
