...
Connect a backend agent and return verified results to the live conversation.
...
Read more about steering the live model for delegation and tools in the prompting guide.

## Choose a delegation mode

With Responses delegation, GPT-Live calls the Responses model you choose, supplies conversation context, and returns backend results to the live conversation. With client delegation, your application prepares the context, runs an agent or workflow, and sends results back to GPT-Live.
...
In both modes, your application manages task state and enforces permissions and required confirmations before running its custom tools. Reviewing backend results is a separate decision: it does not approve every word GPT-Live speaks or guarantee silence while validation runs. See Control playback when needed.

Client delegation also requires your application to maintain conversation context. The delegation event contains metadata, not task text; use transcript events and application state to prepare the backend request.

Compare latency, task success, and cost on your own workload when evaluating your voice agent. For guidance specific to your existing architecture, see Migrate to GPT-Live.

Choose the mode when you create the session; to change modes, start a new session.

## Configure Responses delegation

Add this delegation configuration when creating your Live session. Choose the Responses model independently of the voice model:
...
Start with GPT-5.6 Terra, or try GPT-5.6 Luna for cost-sensitive workloads. Compare answer quality and latency on your tasks before choosing a backend model.
...
With client delegation, return the result directly to GPT-Live. With Responses delegation, follow the function-result flow to continue backend work.

SDK event examples below use `connection`, a connected primary Live WebSocket or sideband from the connection guides. Call the helper after `session.started` on a primary connection; an attached sideband already belongs to a running session.
...
In either delegation mode, use `session.thinking.append` with `delegation_id: null` for background context updates. Keep full HTML, DOM trees, large JSON payloads, and interaction logs in your application or backend. Treat page content as reference data, not instructions.
...
With Responses delegation, configure a vision-capable backend model. Queue a supported Responses image input item with `response.item.create`, then send `response.create` to run or resume backend work. Return all required pending function results before continuing. See Handle Responses delegation.

With client delegation, send visual input to the backend that handles delegated requests, alongside the relevant conversation and application state. Return concise findings using the client result flow.

Keep backend image input separate from `session.input`, which seeds the Live frontend with text history at startup. See Images and vision for supported image formats and model limitations.

## Reduce backend latency

Reduce the time between a request for backend work and a useful result for the conversation. Measure latency at each stage to locate delays. Compare useful spoken response time and task success on the same scenarios, and see the voice agent evaluation Cookbook for evaluation guidance.
...
Update supported settings during the session with `session.update`. Your custom tools still run in your application, so slow service calls, queues, and tool-result buffering can delay the answer even when Live manages the Responses connection. Return each required tool result promptly and continue the backend response.
...
- Reuse backend connections. Keep the API client and its connection pool alive across delegations. For repeated Responses calls, consider a persistent Responses WebSocket.
- Prepare known configuration. Initialize instructions, tools, and connections before the first request needs them. Responses WebSocket mode also supports warming up known request state before generation; follow its setup guidance.
...
- Run guardrails. Check the growing transcript for requests or responses that need intervention. See Apply conversation guardrails.
...
For browser applications, use the WebRTC data channel for captions and local UI updates. When transcript processing runs on your server—for guardrails, lightweight model checks, or speculative tool calls—use a sideband WebSocket to receive events and steer the same GPT-Live session directly.
...
For updates outside a client delegation, use `delegation_id: null`. These appends steer the live model; your application controls UI changes, tool execution, and cancellation. See Send the right kind of update for append examples.
...
See Latency optimization for general Responses guidance and Prompt caching for reusing stable input.
...
Use Evaluating voice agents for repeatable tests. For an existing Realtime tool loop or chained backend, follow Migrate to GPT-Live.
