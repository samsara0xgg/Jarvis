...
## Progress
- slice 1 fold + presenter + row query — 721f12b — jarvis/state/inherent_view.py (bounded ActionView fold, CancelRequestView, single confirmation slot with lazy expiry, response-lifecycle join, widened ChangeKind and checkpoint), jarvis/surface/inherent_presenter.py (four change payloads, two snapshot item shapes, per-section paging, data-driven section_order/counts, `_LIFECYCLE_PENDING` deleted), jarvis/runtime/inherent_view_sequencer.py (row query over every `FOLD_EVENT_TYPES` type plus `source_event_id` and `correlation_json`, carried through a `ViewRow`); 20/20 new checks in tests/integration/test_inherent_action_view.py plus 17 sequencer / 10 flow-control / 27 gated-cancel / 7 correlation unchanged (721f12b's body transposes the sequencer and gated-cancel labels; the 81 total and every gate count in it are the measured ones), full hermetic 1010 passed / 64 deselected (integration baseline 990/64 + this card's 20), lint-imports KEPT (1/1), ruff clean 236 files, mypy strict 236 files. Layer grep: `inherent_view.py` names only `jarvis.shared.realtime` at runtime; `inherent_presenter.py` names no `jarvis.` module at runtime (its L2 value types are TYPE_CHECKING-only, as shipped).
- decisions taken inside the card's boundaries: (a) rows that move only fold-internal truth — `gate.evaluated`, `worker.quiesced`, the cleanup pair, and a `response.*` for a response this fold never opened — return no transition at all rather than an empty durable envelope, the same posture `surface.user_intent` already had, so the wire is unchanged where no wire field is; (b) `RECENT_TERMINAL_ACTION_LIMIT` (20) evicts oldest terminal actions, mirroring the group bound, because D13 specifies a *bounded* ActionViewProjection and the card left the bound implicit — without it every action ever proposed would ride every snapshot forever; (c) the gate verdict is joined to its cancel request by `payload.action_id` and, when that optional key is absent, by `events.source_event_id`, which is the column the card widened the query for; (d) a lifecycle row bumps the response's `revision`, which is what the snapshot reports as the adopter's `lifecycleRevision`.
- slice 2 daemon smoke + flag-off proof — a762d59 — scripts/smoke_inherent_sequencer.py gains `--actions` (seeds one turn, one dispatched action and one live ask before connecting, then drives the action to `result_observed` and supersedes + accepts the ask) and reads snapshot pages from `counts` across every advertised section. Daemon from this worktree on 127.0.0.1:8052 with an overlay carrying realtime.enabled + the flag (log: `inherent_view_sequencer started (scan_cursor=0, projected=0, bus=True)`): snapshot.begin through_cursor=6 section_order=['response_groups', 'actions', 'pending_confirmation'] counts={'response_groups': 1, 'actions': 1, 'pending_confirmation': 1}; the actions page carried exactly the eight ActionUpsert keys and the pending_confirmation page exactly the nine ConfirmationUpsert keys; snapshot.end content_hash=14bed6a2e4ce2a4958e80c585de539c4e3ccedee952995bb768199f4de79cb49 verified=ok; live view.delta cursors 7 action.upsert(running, cancellable=True), 8 action.upsert(result_observed, revision=8, cancellable=False), 9 carrying confirmation.cleared(superseded) then confirmation.upsert in one delta, 10 confirmation.cleared(accepted); exit 0. Flag-off proof: default-config daemons on 8053 (this worktree) and 8054 (a `git archive realtime-integration` export), same three fixed `surface.response_*` rows, 4 v1 frames each, byte-identical (diff empty, 479 bytes), zero sequencer log lines on both. No live LLM run required: no LLM and no external runtime is involved, every event is written directly. Swift 174/174 unchanged (no file under desktop/inherent-swift touched).
- slice 3 ADR sync — f40673c — docs/adr/0014-inherent-realtime-ux.md D13 built sentence + the four pinned facts, D14 the not-yet-built terminalizer note, D3 which ResponseRun lifecycle states have a committed source, and §12's two ownership lines (`inherent_view.py` now owns the action and confirmation folds; `projections.py` is untouched and stays the provenance). Docs to sync disposition: D13 updated, D14 updated, ADR :226-231 updated (the states with no committed source are load-bearing for this fold), docs/spec.html §3.6.7 judged unchanged — it already says Inherent displays open actions and that durable projection truth arrives only through the sequencer's snapshot + view.delta, and this card moves no ownership boundary the spec owns.
- slice 4 verifier pass — 7497fc8 / d4a506c / 266e9f4 — verifier (fresh context, opus, f914d20..HEAD; the nominal realtime-integration..HEAD range also shows three docs/goals cards as deleted, an artifact of the base branch gaining them after this branch's merge base). One confirmed defect, fixed: the projection was bounded only past its four canonical terminals, so every action the Pre-action Gate refused or held for confirmation (jarvis/decision/__init__.py:2039-2079 returns without dispatching) stayed `proposed` forever — and since the sequencer re-folds from cursor 0 on every boot, the leak grew with log history, not process uptime (500 refused proposals measured at 2 snapshot pages / 83083 bytes). d4a506c retires an action at a non-passing `pre_action` verdict, keeps its CancelRequestView (D13 keeps a rejected request visible), tracks a cancel trail only against a target the fold holds, and narrows the second-request block to received/authorized/quiescing per D13. Its second confirmed item (the acceptance never observed `received`) and three advisories are fixed in the same commit: `received` is now asserted between the request row and its verdict, the request's own `action.failed` terminal is covered, and the real-Event-Log catch-up asserts `revision == 2`, the row's actual events.id. 266e9f4 fixes the two ADR self-contradictions it found (D13 still said L3 computes `cancellable_hint`; D14 still said the presenter reads `PendingConfirmations` directly). Left as advisories: the content-hash case is close to a tautology on its own (the real ordering constraint is pinned by the section-order and page_index cases plus RealtimeTransport.swift:118-131); the row query now decodes every `gate.evaluated` payload and scans the (bounded) cancel map per pre_action verdict, which is O(cancels) per row on a boot re-fold. Final gates after the fix: lint-imports KEPT (1/1), ruff clean 238 files, mypy strict 236 files, full hermetic 1013 passed / 64 deselected in 44.27s, 23/23 in the card's acceptance file, Swift 174/174. Daemon smoke re-run on the fixed code (127.0.0.1:8052, flag on): same canaries — section_order all three sections, action.upsert(result_observed) revision=8, confirmation.cleared(superseded) + confirmation.upsert in one delta at cursor 9, confirmation.cleared(accepted) at 10, exit 0.
