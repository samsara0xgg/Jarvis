...
## Progress
- slice 1 fold + presenter + row query — 721f12b — jarvis/state/inherent_view.py (bounded ActionView fold, CancelRequestView, single confirmation slot with lazy expiry, response-lifecycle join, widened ChangeKind and checkpoint), jarvis/surface/inherent_presenter.py (four change payloads, two snapshot item shapes, per-section paging, data-driven section_order/counts, `_LIFECYCLE_PENDING` deleted), jarvis/runtime/inherent_view_sequencer.py (row query over every `FOLD_EVENT_TYPES` type plus `source_event_id` and `correlation_json`, carried through a `ViewRow`); 20/20 new checks in tests/integration/test_inherent_action_view.py plus 27 sequencer / 10 flow-control / 17 gated-cancel / 7 correlation unchanged, full hermetic 1010 passed / 64 deselected (integration baseline 990/64 + this card's 20), lint-imports KEPT (1/1), ruff clean 236 files, mypy strict 236 files. Layer grep: `inherent_view.py` names only `jarvis.shared.realtime` at runtime; `inherent_presenter.py` names no `jarvis.` module at runtime (its L2 value types are TYPE_CHECKING-only, as shipped).
