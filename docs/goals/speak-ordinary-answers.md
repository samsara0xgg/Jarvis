...
## Progress
- Slice 1 — L3 default `voice_notify` (reconciliation terminals keyed on trigger stay `queue_review`), F1 + exhausted-retry refusals `voice_notify`, runtime comment corrected, hermetic test `tests/integration/test_attention_policy_default_channel.py`; Tier 1 green (716 passed / 63 deselected).
- Slice 2 — live run from this worktree (daemon on 127.0.0.1:8007, root ~/.jarvis-card-speak, DeepSeek fast preset): turn T7ca822e9, `surface.response_open` for `RESPcb25e9d458c14ab686ed234a0c7a66e2` carries `attention_channel: voice_notify`; `surface.playback_started` (events.id 54) plus four `surface.playback_segment_prepared` rows for that response_id. Docs synced: spec §12.4 + §3.6.4 (one sentence each), ADR-0009 D4 amendment; ADR-0002 judged unchanged (its :1102-1108 table maps channel -> surface and its Limitation amendment pins timeout/failed -> queue_review, both still true).
- Slice 3 — verifier follow-ups: spec §12.4 now names all three reconciliation terminals (`action.cancelled` was missing), and `tests/integration/test_attention_policy_default_channel.py` pins the routing one layer up, through the real `decide()` — a sweep-shaped `action.timeout_assumed` trigger returns `queue_review`, a plain `surface.user_intent` returns `voice_notify`. Mutation-checked (emptying `_RECONCILIATION_TRIGGER_TYPES` fails the new test). Tier 1 green (717 passed / 63 deselected).
