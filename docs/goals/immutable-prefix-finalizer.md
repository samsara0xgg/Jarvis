...
- Verifier pass (no blocker, 5 nits) — 83bc855 + docs commit — added `suffix_risk` to `StreamFinalizationFailure`, pinned `response.cancelled.committed_prefix_hash` in the restart test, documented `next_segment_sequence` as the next chunk sequence, reconciled D3 wording with D1's in-memory copy; kept: finalizer classifies the suffix only (D3 "may inspect" the full answer, subject is structurally "none"); re-run: finalizer 5 passed, hermetic 720 passed / 63 deselected, lint-imports KEPT, ruff clean (204 files), mypy strict clean (202 files).
