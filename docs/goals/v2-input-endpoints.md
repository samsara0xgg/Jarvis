...
## Progress
- slice 1 L2 inbox — 977d1db — jarvis/state/input_submission_inbox.py (receipt table keyed by principal + client_instance_id + request_id, one BEGIN IMMEDIATE appending the canonical surface.user_intent with the server-minted turn_id / source_client_request_id / source_surface beside the receipt insert, D21 ASR processing lease with resume-by-lookup on an expired lease, PayloadConflictError, release for a failed run), tests/integration/test_input_submission_inbox.py; 14/14 pass in 0.06s, layer grep names only jarvis.state.event_log, lint-imports KEPT (1/1), ruff clean (231 files), mypy strict 229 files. Integration baseline re-confirmed on this worktree after the merge: 922 passed / 64 deselected in 42.18s.
- slice 2 L5 routes + wiring — 0d1aff1 — jarvis/surface/inherent_protocol.py (SubmitV2Request / AsrSubmitV2Request over a shared request_id + client_instance_id + client_created_at_ms base, SubmitV2Response / AsrSubmitV2Response), jarvis/surface/inherent_server.py (both routes registered only inside the `deps.v2 is not None` block, `_v2_authorize` reusing `_v2_presented_token` + `token_matches` for 403, `InputSubmissionOutcome` as the L2->L5 value, the ASR bounds re-checked in v1's order against the same constants and decoder), jarvis/runtime/inherent_loop.py (`V2_PRINCIPAL`, `_submit_text_v2`, `_submit_asr_v2` holding no transaction across ASR and releasing a failed lease), tests/integration/test_inherent_submit_v2.py; 17/17 pass, four regression files 54 passed / 1 skipped (live_llm), lint-imports KEPT (1/1), ruff clean (232 files), mypy strict 230 files. v1 routes, `_run_asr_submit`'s body, `jarvis/surface/voice_*` and `/inherent/image-submit` untouched.
- slice 2b H1 conformance — 2498846 — tests/canary/test_no_projection_writes.py registers jarvis/state/input_submission_inbox.py as the owner of input_submission_receipts for INSERT and adds the same per-owner allowlist for the lease-resolving UPDATE (the carve-out authorized_dispatch_outbox's admission CAS already has); release_asr_request now marks the lease 'released' instead of deleting the row, so no DELETE exemption is needed and D21's "an identical re-upload may resume the same request" keeps the original turn_id. 39/39 across the four touched files.
- slice 3 request-to-group correlation — b3fa8cb — jarvis/state/inherent_view.py (the turn_id -> source_client_request_id map fed by surface.user_intent before the relevance check, stamped on ResponseView and the first-sight ResponseGroupView, carried in InherentViewCheckpoint, capped at PENDING_REQUEST_LIMIT=64), jarvis/runtime/inherent_view_sequencer.py (one line: the drain query selects surface.user_intent too — without it the fold never sees the row; the fold returns None for it so no frame is produced and the wire is unchanged), jarvis/surface/inherent_presenter.py, tests/integration/test_inherent_view_correlation.py; 7/7 pass, full hermetic 960 passed / 64 deselected in 42.68s (922 baseline + 14 + 17 + 7), lint-imports KEPT (1/1), ruff clean (233 files), mypy strict 231 files. test_inherent_sequencer.py, test_inherent_server_asr_submit.py and test_serve_inherent_smoke.py are untouched and pass.
- slice 4 Swift input client + pending input — 4f661e6 — InherentRealtime/InputSubmissionClient.swift (injected transport, UUID request_id, injected client_instance_id, one byte-identical retry on a lost answer, typed .payloadConflict/.unauthorized with no second send, SHA-256 over the exact uploaded bytes in the multipart body; body rendering is an extension on the module's existing JSONValue, not a second JSON type), RealtimeViewDTOs.swift (optional sourceClientRequestId), RealtimeState.swift (PendingInputState + pendingInputs keyed by request_id, bounded at pendingInputLimit=32), RealtimeEffects.swift (.inputSubmitted), RealtimeReducer.swift (store + evict oldest past the bound; applyOpened clears a matching entry and ignores an unknown id), InputSubmissionClientTests.swift, RealtimeTestFixtures.swift (opened takes sourceRequest); Swift 174/174 pass, 0 failures, in 11.05s (165 baseline + 9 new). RealtimeTransportV2.enabled still false; InherentCard app target, BridgeBackend and NativeCardModel untouched.
- slice 5 live smoke + live run — 507b93f — scripts/smoke_inherent_input_v2.py. Daemon from this worktree on 127.0.0.1:8052, own runtime root ($CLAUDE_JOB_DIR/tmp/lane-c-live) and an overlay carrying realtime.enabled + realtime.inherent.v2_sequencer.enabled, presets unchanged (cost.recorded model=deepseek-v4-flash, real answer returned). Raw output: server.hello connection_id=Cfdbc4582... log_epoch=L3da456e3..., transport.ack through_cursor=0; submit/v2 200 receipt {request_id R6cb9898b420c4a218e355752c4b8d9e0, input_event_uid 078642edcdfd45c0ae5b4d2fcdb02182, turn_id T60f3258e, session_id null}; byte-identical retry 200 with the identical receipt; same request_id + different text -> 409 "request_id already submitted with a different payload"; view.delta event_cursor=6 response.opened response_group_id=RGRP550795b1e04d551c8344ef8ba86e1d02 turn_id=T60f3258e source_client_request_id=R6cb9898b...; surface.response_emitted id=11 for that turn; asr-submit/v2 200 (zh.wav, 178988 bytes) text="开放时间早上9点至下午5点。" turn_id=Tc0472d5e; utterance.received id=12 carrying Tc0472d5e. Log check: exactly one surface.user_intent (id=1) and one utterance.received despite the retry; turn.started id=2 carries T60f3258e, i.e. claim_input_once reused the receipt's turn_id live. Canary chain printed: R6cb9898b... -> 078642ed... -> T60f3258e -> RGRP550795b1.... Audio was routed to BlackHole 16ch for the run and restored to MacBook Pro Speakers afterwards (verified with `SwitchAudioSource -c -t output`); the daemon on 8006 was never touched and still answers /api/health. Hub's later audio-guard rule (never leave the default on BlackHole; restore to MacBook Pro Speakers) is satisfied by this run's end state; a further live run here would skip the switch entirely, since both submissions are HTTP.
- slice 6 docs sync — docs/adr/0014-inherent-realtime-ux.md two corrections: D21's "Until this route exists the advertised capability remains false" is now scoped to the image route and says the text and ASR routes are built (image_input stays False at inherent_loop.py:2773, verified); `claim_turn_once(trigger_event_uid)` is corrected to `claim_input_once(trigger_event)` with a note that the old name was this ADR's first draft. Judged unchanged and not edited: §12 New Python files (`jarvis/state/input_submission_inbox.py` already names exactly what was built) and §12 New repository-owned Swift files (`InputSubmissionClient.swift` already listed with the same responsibility; its `Under desktop/inherent-swift/InherentCard/` heading is stale for every entry in that list since D32 moved the v2 client into the InherentRealtime target, a pre-existing condition this card did not create and does not widen); §14 step 5 (the step description already matches what landed); docs/spec.html (this card adds no layer boundary, ownership rule or persistent-state semantic — surface.user_intent gains two payload keys the registry already accepts Day-1, and the receipt table is a bounded operational table of the kind ADR-0001 H1 already governs, registered with the H1 canary in 2498846).
- accepted note — D21 says an ASR "retry joins/returns the same result"; the inbox returns the stored result for an accepted receipt and resumes an expired or released lease, but refuses a retry that finds a *live* lease with HTTP 503 rather than blocking on the in-flight run. The card is silent on the concurrent-duplicate case and 503 is one of its own listed ASR codes (it is also what the v1 route already returns for busy voice input), so the client's retry after 503 is what joins. Not treated as an ADR deviation.
- slice 7 verifier pass — fd2c537 + the ADR deviation note — verifier (fresh context, opus, corrected range 16ef152..HEAD): implementation judged correct on the D21 contract, route registration/auth parity, the correlation chain, the Swift half and every boundary (v1 routes, _run_asr_submit body, voice_*, InherentCard target, image-submit, event_log.py all zero-diff; RealtimeTransportV2.enabled still false). Two confirmed defects, both fixed. (1) The v2 token was not the first gate: FastAPI validates the body before the handler, so an unauthenticated caller got 422 for a malformed body and 403 only for a well-formed one (schema probing) and a multipart upload was buffered in full before refusal. Auth moved into middleware registered beside the v2 socket, scoped to the two v2 paths so v1 is untouched; four new tests fail without it; live-confirmed on 8053 (no token + malformed body -> 403, bad token + malformed body -> 403, no token + good body -> 403, v1 POST /inherent/submit without a token still 200). (2) ADR-0014 D21's ASR paragraph asserted "appends exactly one utterance.received [in the final transaction]" and "no event existed before the final transaction", both false as built; a "Deviation as built" paragraph now records the real shape (pipeline commits the row on its own connection, duplicate window closed by lookup, one stated lease-TTL edge, and what would retire the deviation). Also fixed from my own re-read and the advisories: the crash-recovery path now stores the committed row's transcript/emotion/utterance_id so a recovered retry no longer answers with empty text (test strengthened); the ASR payload hash now covers `language`, matching the text side (new test); two overstated docstrings corrected (the one-utterance guarantee now states its TTL edge; the stolen-lease paragraph says the `accepted` early return is what prevents a double write); the H1 canary comment no longer claims equivalence with the outbox's byte-exact rule; the Swift multipart test asserts the digest value rather than the field name. Left as owner follow-ups, none blocking: `utterance_id` on the ASR receipt is structurally present but always null (the pipeline is called with four positional args, same reason session_id is a stated non-goal); the Swift snapshot path does not clear a pending input because `ResponseGroupSnapshot` has no `sourceClientRequestId` (Python already sends it; harmless while enabled=false and no UI is wired, worth closing with the UI card); the lease-TTL race in advisory (d) can only be fully closed by making VoicePipeline idempotent per turn_id, which is lane B's file. Final gates: lint-imports KEPT (1/1), ruff clean (234 files), mypy strict 232 files, full hermetic 965 passed / 64 deselected in 42.60s, Swift 174/174, three regression files 24 passed / 1 skipped. Audio: this re-run needed no switch (both submissions are HTTP) per the hub's rule, and on finishing I found the system default left on BlackHole 16ch by another lane and restored it to MacBook Pro Speakers.
