...
- Follow-up, landed as a separate commit AFTER the card's own evidence so condition 5 stays true against the tree it was measured on — 31626a1, on the hub's explicit ruling: the three pre-existing assertions at `SubmitRequestTests.swift:7,52,79` now pass `environment: [:]`, expected values unchanged. Both runs are green at the same count: `env -u JARVIS_INHERENT_BRIDGE_PORT bash scripts/test_inherent_swift.sh` and `JARVIS_INHERENT_BRIDGE_PORT=8009 bash scripts/test_inherent_swift.sh` each report `Executed 189 tests, with 0 failures (0 unexpected)`, where the second gave 3 failures before this commit. `scripts/test_inherent_swift.sh` was deliberately not touched.
