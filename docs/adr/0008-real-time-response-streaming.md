...
Built (refusal half only): `decide_foreground` in `jarvis/decision/response_run.py` is that explicit foreground policy. Same `response_group_id` enqueues after drain; a cross-group candidate takes the lane only when its Event Log row id is later than the incumbent's, and otherwise declines — terminalized with no playback opened, never queued behind another group's drain lane. `make_foreground_decision_callable` (`jarvis/runtime/__init__.py`) injects it into the L5 media actor, which is what keeps `jarvis/surface` free of a `jarvis.decision` import. The "degrades to document/silent delivery" half still waits on the `foreground_output` lease, and no `supersede_foreground` method exists.
