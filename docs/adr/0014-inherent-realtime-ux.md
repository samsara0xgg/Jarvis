...
3. If a durable enqueue would exceed either limit, the server best-effort sends `server.resync_required(reason="client_backpressure")` on the control lane and closes that client with a retryable code. The D8 snapshot handoff no longer trips this rule merely because the plan is large: it enqueues page by page and waits for the sender to free lane depth, inside the one existing adoption deadline and with no budget of its own, so a plan whose total exceeds `durable_bytes` while every individual frame stays under it is delivered and adopted. Snapshot delivery is progressive for exactly that reason and the sequencer's catch-up is not: `complete_snapshot` must stay atomic, because `live_frontier` is set only after its last frame is enqueued and any event committed during a yield there would be dropped forever. After adoption rule 3 keeps its full force.
   A frame whose own encoded size exceeds `durable_bytes` fits no lane depth at all, so it fails immediately — no capacity wait, no deadline spent — with `server.resync_required(reason="frame_over_budget")` on the same control frame and the same close code. That leaves a known ceiling: a single response group whose inline bytes exceed `durable_bytes` cannot be delivered to any client, and D8's artifact-reference remedy is blocked because `DocumentReference.event_uid` names the close-time `surface.response_emitted` row an open response does not have. Lifting the ceiling is an L2 change, not a flow-control one.
...
Built, for the **expiry leg only**: `confirmation.expired` is registered in
`jarvis/state/event_log.py`, the terminalizer is `terminalize_confirmation` in
...
The "sole exit" rule above is therefore not yet fully realized: the accept and
reject legs still append through `emit_event` on the ADR-0012 answer path and
do not pass the terminalizer's CAS, so "at most one terminal row per
`confirmation_id`" holds among terminalizer callers rather than universally.
Routing them through it is a later card.
