...
| D3 | Verify strategy | Composite: L4 raw observations (diff + optional `verify_command`) + L3 reviewer LLM. Evidence level per F2 ladder |
...
ADR-0002's mapping (one row per claim emitted; some conditions emit two
claims — a Postcondition plus a Limitation):

| Condition | Claim type | `relation` | `level` | `task.verified`? |
|---|---|---|---|---|
| `verify_command` present AND exits 0 AND reviewer.verdict=ok | Postcondition | supports | **verified** | **yes** |
| `verify_command` present AND exits 0 AND reviewer.verdict=fail | Postcondition | refutes | executed | no |
| " " (same condition, second claim) | Limitation | limits | executed | — |
| `verify_command` present AND exits ≠ 0 (any verdict) | Postcondition | refutes | executed | no |
| " " (same condition, second claim) | Limitation | limits | executed | — |
| `verify_command` absent AND `diff_nonempty` AND reviewer.verdict=ok | Postcondition | supports | observed | no |
| " " (same condition, second claim — §8.5 rule 6) | Limitation | limits | reported | — |
| `verify_command` absent AND `diff_nonempty` AND reviewer.verdict=fail | Limitation | limits | observed | no |
| `diff_nonempty == False` (Codex produced nothing) | Limitation | limits | reported | no |

Notes:
- Level `executed` (per spec §3.4.11) maps to "tool ran and returned" —
  `verify_command` running to exit code is execution evidence. The
  reviewer's verdict cannot raise the level above `executed` on its own
  because the reviewer is an LLM (Report-grade source); the verdict
  modulates `relation` (supports vs refutes), not `level`.
- Only the supports/verified combination triggers `task.verified` —
  this is the spec §8.8 derivation rule.
- "Postcondition" claims always pair with a contrasting Limitation
  whenever the postcondition is not fully met. Day-2 never silently
  promotes a partial result.
...
- `hermes-agent/repo/agent/transports/codex_app_server.py` lines 75–130
  — `-c` sandbox flag injection pattern (see § Codex contract).
...
drift no longer affects jarvis behavior.
...
unit-tested against ~10 weird-path fixtures.

Lifecycle:

1. `CodexAppServerClient(...).initialize()` (timeout 5s).
2. **Inject `submit_report` MCP tool** before `thread/start` per spec
   §3.5.8 (see § submit_report injection below).
3. `request("thread/start", {"cwd": str(cwd)})` → `thread_id`.
4. `request("turn/start", {"threadId": ..., "input": [{"type":"text",
   "text": task_goal}]})`.
5. Loop `take_notification(timeout=0.25)` until `method ==
   "turn/completed"` or deadline.
6. Every 30s wall-clock → `on_heartbeat({summary, elapsed_ms,
   last_item_summary})`.
7. On `turn/completed` AND a captured `submit_report` tool call → use
...
contract); jarvis must add it. The injection mechanism is Codex's MCP
tool registration:

```python
# jarvis/execution/codex_action.py — submit_report injection sketch
SUBMIT_REPORT_TOOL = {
    "name": "submit_report",
    "description": (
...
# Codex app-server registers via JSON-RPC:
#   request("mcp/register_tool", {"thread_id": ..., "tool": SUBMIT_REPORT_TOOL})
# The exact JSON-RPC method follows Codex 0.125.0+; verify against
# `codex --help` and Hermes' codex_app_server.py before locking.
```

Capture pattern: the take_notification loop watches for `item/tool_call`
notifications with `tool_name == "submit_report"`. The arguments become
the canonical `WorkerReport` payload. If `turn/completed` fires without
a captured submit_report, the L4 handler emits `worker.report_missing`
and a Limitation Claim per spec §3.5.8. The worker's natural-language
final_text is **not** used as a substitute — spec rule: "Worker report
永远只是 Report Claim; Jarvis 还要自己检查 artifact / test /
postcondition" (§3.5.8 last bullet). The `git diff` capture + verify
command in L4 raw observation are jarvis's independent evidence.
...
### Verify_diff contract (L4 observation, per spec §3.4.11)

```python
# jarvis/execution/tools.py (verify_diff_handler sketch)
@dataclass(frozen=True)
class VerifyDiffRawResult:
    diff_text: str
    diff_nonempty: bool
    verify_command: str | None
    verify_command_exit: int | None         # None if absent
    verify_command_stdout_tail: str          # last ~2KB
    verify_command_duration_ms: int | None

def verify_diff_handler(action_request: ActionRequest) -> RawResult:
    """L4 observation handler.
    Reads diff.txt artifact, optionally runs verify_command in cwd=repo_path
    with timeout 600s, captures exit code + stdout tail. Returns
    RawResult whose ToolDefinition.result_semantics == "observation" per
    spec §3.4.11 canonical enum. NO reviewer call. NO verdict.
    NO apply-check. L3 emits action.result_observed from this RawResult.
    """
```

L3 Result Interpreter consumes `action.result_observed` whose
`result_semantics="observation"`, calls `review_diff(...)`, then forms
the `(relation, level)` Evidence pair per the F2 ladder.
...
`verify_command` join the optional set):
...
| `jarvis/execution/codex_action.py` | ~120 | New (incl. submit_report injection) |
...
- `jarvis/execution/tools.py` — replace `spawn_worker_handler` body
  (real Codex + submit_report injection + heartbeat loop + sandbox
  `-c` injection + dirty-tree auto-stash); replace `verify_diff_handler`
  body with observation handler (returns RawResult with
  `result_semantics="observation"`; no reviewer); register new
...
- `jarvis/decision/__init__.py` — emit `cost.recorded` after every LLM
  ...
  registry owner_layer=L3); wire reviewer call inside Result
  Interpreter on `action.result_observed(result_semantics="observation")`;
  form Evidence `(relation, level)` per the F2 ladder; populate
  ResponsePlan with canonical `output_risk_class` +
  `required_gate_mode` per spec §3.4.13.
...
- **L3 Decision** — resolver time-window query **via projection API
  ...
  (F4 fix); Result Interpreter consumes
  `action.result_observed(result_semantics="observation")`, calls
  reviewer, combines with `verify_command` result, forms Evidence
  `(relation, level)` per ladder; emits `cost.recorded` for both
  decision LLM rounds and Codex turns (L4 returns cost in
  RawResult.metadata). ResponsePlan carries canonical
  `output_risk_class` + `required_gate_mode` per spec §3.4.13.
- **L4 Execution** — `codex_client.py` (vendored), `codex_action.py`
  (incl. submit_report tool injection per spec §3.5.8),
  `diff_capture.py` (incl. dirty-tree auto-stash), real
  `spawn_worker_handler`, observation `verify_diff_handler`
  (returns RawResult with `result_semantics="observation"`, no reviewer,
  no apply-check), new `create_task_handler`.
...
| 40+N | `action.result_observed` | evt 39+N | turn_id=T2, action_id=A4 | result_semantics=observation, diff_text_preview, diff_nonempty=True, verify_command="pytest", verify_command_exit=0, verify_command_stdout_tail | observation |
| | *[L3 Result Interpreter calls `jarvis.decision.reviewer.review_diff(...)`. No event for the reviewer invocation itself; the next `cost.recorded` records its LLM use.]* | | | | |
| 41+N | `cost.recorded` | — | turn_id=T2, run_id=R_Y | kind=reviewer, model=gpt-5.5, tokens_in/out | none |
| 42+N | `claim.created` | evt 40+N | task_id=T_X, claim_id=C2 | type=Postcondition, statement="diff implements the task goal and verify_command passed" | postcondition |
| 43+N | `evidence.attached` | evt 42+N | claim_id=C2, evidence_id=E2 | relation=supports, level=verified, source_type=tool, source_id=verify_diff, artifact_ref=diff_path | postcondition |
| 44+N | `task.verified` | evt 43+N | task_id=T_X | by=jarvis_runtime | verification |
| 45+N | `cost.recorded` | — | turn_id=T2 | kind=decision (L3 LLM 3: final response) | none |
| 46+N | `action.proposed` | evt 44+N | turn_id=T2, action_id=A5 | tool=response | none |
| 47+N | `gate.evaluated` | evt 46+N | turn_id=T2, action_id=A5 | gate=pre_emit, outcome=allow_completion_language, output_risk_class=consequential_claim, required_gate_mode=full_text, active_claim_levels=[verified] | none |
| 48+N | `surface.response_emitted` | evt 47+N | turn_id=T2 | text, voice_text, document_text, delivered_via=["voice","banner","stdout"], attention_channel=voice_notify | none |
| 49+N | `turn.ended` | evt 16 | turn_id=T2 | duration_ms | none |

Total Phase-2 events (excluding heartbeats): 28 + N (was 26+N before
the §3.5.9 correction added the missing Report Claim + evidence pair,
and the canonical terminal `action.result_observed` for the long-run
worker action). For N=6 → 34 events. **Tier-2 acceptance count
tolerance: A1 widens to [30, 42] for the Phase-2 run.**
...
Events 1–(38+N) identical to the happy path. At evt 38+N,
`verify_command_exit ≠ 0`; the L3 Result Interpreter follows the
ladder → `evidence.attached.level == "observed"` (not `verified`); evt
42+N (`task.verified`) is **not** emitted; an additional Limitation
Claim is emitted alongside the Postcondition; at evt 45+N, the Pre-emit
Gate verdict is `force_limitation_language`; at evt 46+N, the surface
response matches one of `{r"reported,?\s*not\s+verified", r"测试.{0,4}没过",
r"未验证"}`.
...
|---|---|---|---|
| 1 | LLM-free, < 45s wall | Inherited | + cost-recorded canary; + codex_version preflight unit test; + ladder mapping unit test; + ack-before-fork canary; + 8 new event types in registry |
...
- New canary tests:
  ...
  - `test_canary_submit_report_injection` — AST scan:
    `jarvis/execution/codex_action.py` registers a
    `submit_report` tool descriptor and the take_notification loop
    captures `item/tool_call` events with that tool name.
...
Wall-time budget: < 60s (was < 30s Day-1; +30s for the larger AST
canary suite + new unit tests; submit_report + sleep_wake unit tests
add ~10s; relation-column + dirty-tree fixtures add ~5s).
...
| ID | Invariant |
|---|---|
| L1 | `task.created` without `verify_command` + scenario runs → assert **no** `task.verified` emitted; assert `evidence.attached.level == "observed"` for the Postcondition Claim (reviewer.verdict=ok, diff_nonempty) |
| L2 | `task.created` with `verify_command="pytest"` + verify_command exit=0 + reviewer.verdict=ok → assert `task.verified` emitted; assert `evidence.attached.level == "verified"` |
| L3 | `task.created` with `verify_command="pytest"` + verify_command exit ≠ 0 → assert **no** `task.verified`; assert `evidence.attached.level == "observed"`; assert at least one Limitation Claim; assert `surface.response_emitted.text` matches regex set `{r"reported,?\s*not\s+verified", r"测试.{0,4}没过", r"未验证"}` |
| L4 | Codex produced empty diff (`diff_nonempty == False`) → assert `evidence.attached.level == "reported"`; assert Limitation Claim emitted; assert **no** `task.verified` |
| L5 | reviewer-fail path (`verify_command` absent + reviewer.verdict="fail" + `diff_nonempty == True`) → assert `evidence.attached.level == "reported"`; assert Limitation Claim emitted; assert **no** `task.verified` |
...
- **A. Event Log structural** — registry now has Day-1 count + 12 new
  ...
  widens to **[30, 42]** for Phase-2 (was [28, 40]; +2 covers the
...
- **H. Anti-bypass canaries** — twelve new entries (cost-recorded
  ...
  submit-report-injection, regex-constants-single-source,
  pricing-at-shared).
...
| 6 | `codex_action.py` one-shot driver around `codex_client`; include `-c` flag injection per F9 (`model`, `model_reasoning_effort`, `sandbox_mode`, `writable_roots` via `_toml_list_quote`); register `submit_report` MCP tool before `thread/start`; capture `item/tool_call` events with `tool_name == "submit_report"` in the take_notification loop | Hermes `codex_app_server.py:75-130` for `-c` pattern; spec §3.5.8 for submit_report | Canary `test_canary_submit_report_injection`; unit test (mocked JSON-RPC): assert all four `-c` flags + submit_report tool registration appear in spawn flow |
| 7 | `diff_capture.py`: dirty-tree auto-stash before Codex (`git stash push -u`), `git -C cwd diff` capture after, `git stash pop` with conflict-as-artifact fallback. **No** `git apply --check` | F3; § Dirty-tree policy | Unit test: tmp git repo fixture; clean tree path; dirty-tree-clean-pop path; dirty-tree-conflict path (conflict written as artifact + Limitation Claim) |
| 8 | `jarvis/decision/reviewer.py` (L3): structured-output LLM call, fresh-context system prompt (via `LLMClient.fresh_context()`), `ReviewerVerdict` dataclass | F4; spec.html §13.2 I10 ("claim ≤ evidence") | Canary `test_canary_reviewer_in_l3` + `test_canary_reviewer_fresh_context`; unit test (mocked LLM): `tests/unit/test_reviewer.py` covers ok / fail / malformed JSON / token recording / fresh-context |
| 9 | Replace `spawn_worker_handler` stub: real Codex flow + submit_report capture + heartbeat loop + sandbox `-c` injection + diff capture (with dirty-tree stash) + `worker.*` event emission + `task.executor_assigned` / `task.executor_reported`. On missing submit_report → emit `worker.report_missing` + Limitation Claim per spec §3.5.8. Returns RawResult with `result_semantics="report"` and `metadata.cost` populated from Codex turn tokens | Day-1 `jarvis/execution/tools.py` `spawn_worker` stub; Hermes `codex_app_server_session.py` | Unit test (mocked subprocess + clock): `test_spawn_worker_real`; missing-submit-report path; J-tier covers live path |
| 10 | Replace `verify_diff_handler` with **observation** handler: runs `verify_command` if present (timeout 600s, cwd=repo_path), captures exit/stdout-tail; returns RawResult with `result_semantics="observation"`. **No** reviewer call. **No** apply-check | F3, F4; spec §3.4.11 result_semantics enum | Canary `test_canary_no_apply_check`; unit test: `tests/unit/test_verify_diff_observation.py` covers `verify_command` present/absent, exit 0/non-zero, timeout |
| 11 | L3 Result Interpreter: consume `action.result_observed(result_semantics="observation")`; call `jarvis.decision.reviewer.review_diff(...)`; combine reviewer verdict + `verify_command` result per F2 ladder; emit `claim.created` (Postcondition or Limitation) + `evidence.attached` with `(relation, level)` per spec §8.6; emit `task.verified` only when level=verified. Per spec §8.5 rule 6, missing verify_command itself produces a Limitation Claim. ResponsePlan carries `output_risk_class` + `required_gate_mode` per spec §3.4.13 | F4, F2 ladder; spec.html §8.4 + §8.6 + §3.4.13 | Canary `test_canary_evidence_relation_required`; unit test (LLM-free): `tests/unit/test_result_interpreter_ladder.py` parametrized over all ladder rows |
| 12 | `pre_emit_phrases.py`: single source of truth for `LIMITATION_REGEXES` + `COMPLETION_REGEXES`; ADR-0001 F4 / F5, ADR-0002 K5 / L3 all reference these constants | § Canonical limitation phrasing | Canary `test_canary_regex_constants_single_source`; unit test: regex constants compile + match seeded text samples |
| 13 | `notify.py` (`say` + `osascript`); document-channel banner truncation at 240 chars; explicit Attention Channel → Physical Surface mapping per § Notification contract | legacy `core/media_ducking.py:151-159` (osascript shape); spec §3.4.7 + §12.4 | Unit test (mocked subprocess): `test_notify`; truncation + escaping; channel-mapping table parametrized |
| 14 | `daemon.py` fork-detach: double-fork + setsid; child stdin/stdout/stderr → /dev/null; cwd=/ | POSIX double-fork pattern | Unit test (platform-skipped on non-Mac): `test_daemon`; parent returns "parent" immediately, child returns "child" with redirected fds |
| 15 | `sleep_wake.py`: install macOS IOPM notification observer; before-sleep + on-wake callbacks emit `mac.sleeping` / `worker.suspended_by_sleep` / `mac.awake` / run reconcile_after_wake() per spec §3.7.8. Idempotent reconciliation | spec §3.7.8; Mac-only architecture | Unit test (stubbed observer + fake clock): induced sleep/wake cycle emits the four-event sequence; reconcile_after_wake() is idempotent (K7 / K8) |
| 16 | CLI entry point: regex classifier (`_LONG_RUN_RE`), ack-before-fork, fork-detach, child re-bootstrap (incl. `install_power_observer(...)`). **No** SQLite open before fork. **No** task-bound check in classifier | F5; spec §3.7.8 (child must install power observer) | Canary `test_canary_daemon_ack_before_fork`; integration: parent exits within 100ms; child's bootstrap registers sleep observer |
| 17 | Surface render: voice → `say`, document → `notify`; populate `delivered_via` (physical) + `attention_channel` (logical L3 channel) on `surface.response_emitted` | F6; § Attention channel mapping | Unit test: payload populated correctly for all-channel and partial cases |
| 18 | Full Tier 1 canary sweep (12 new canaries from § Tier 1 list) | ADR-0001 H series canary style | Full Tier 1 green (< 60s) |
| 19 | Tier 2 J + K + L acceptance tests (gated by `--live-codex` + `--live-llm`); includes D-1 fixture seeding + D-day happy path + verify-fail + empty-diff + crash + timeout + simulated sleep/wake + missing-submit-report | this ADR § Acceptance | All J (1-13) / K (1-8) / L (1-5) invariants pass on Allen's Mac |
| 20 | `docs/progress.md` ADR-0002 acceptance summary; update CLAUDE.md anchor if needed | ADR-0001 build-order convention | Doc-only; Tier 1 docs-only |

**Estimated commit count: 21.** Each step ~50–300 LOC delta. Worktree
...
- 8 new canonical event types fill gaps spec.html §5.4 flagged (most
...
- All ADR-0001 invariants (claim ≤ evidence, no entity_id invention, no
  ...
  unchanged.
- Tier 1 < 30s budget is loosened to < 60s (was 45s in the draft; +15s
  for sleep/wake + submit_report + relation-column + dirty-tree
  fixtures). If this slips, the new AST canaries + sleep/wake fake-clock
  fixtures are the prime suspects.
...
6. **~~Codex's own `submit_report` injection~~** — **Resolved.** Spec
   §3.5.8 mandates worker wrapper injects `submit_report` tool. Day-2
   implements per § submit_report injection: jarvis registers
   `submit_report` MCP tool with Codex thread before `turn/start`,
   captures `item/tool_call` events with `tool_name == "submit_report"`
   in the take_notification loop, and falls back to `worker.report_missing`
   + Limitation Claim if Codex completes the turn without calling it.
   The captured WorkerReport is the authoritative `worker.reported`
   payload; jarvis still independently inspects diff + verify_command
   per §3.5.8 last bullet ("Worker report 永远只是 Report Claim").
...
10. **(New)** **Cross-Codex-version forward compatibility** — Day-2
    pins Codex ≥ 0.125.0 via preflight (J1). When Codex 0.130 changes
    the JSON-RPC method name for tool registration, this ADR will need
    a v1.1 patch. Day-N task.
11. **(New)** **Codex 's `model_reasoning_effort=xhigh` cost** — One
    flagship run is ~$0.50–$2.00 (rough). `cost.recorded` provides
    observability, but no per-task ceiling is enforced. Day-N adds
    budget gates if usage runs hot.
...
- The 18-step build order is converted to a `progress.md` checklist.
