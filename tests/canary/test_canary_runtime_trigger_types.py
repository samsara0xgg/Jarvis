...
- ``action.timeout_assumed`` (supervisor sweep deadline)
- ``action.failed`` (dispatch failure)
- ``action.cancelled`` (a waiter blind to it sleeps out its whole
  trigger timeout on a turn whose action is already durably over)
...
def test_canary_runtime_trigger_types_exact_set() -> None:
    ...
    assert actual == _EXPECTED_TRIGGER_TYPES, (
        ...
        "runtime waiter deadlocked on the terminal failure "
