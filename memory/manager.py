...
字段说明：
- key：简短语义标识，同类型同key的记忆视为同一件事的更新（event/task 为 null）
  例：location, favorite_drink, sister, wifi_password
- expires：过期日期，仅 event/task 使用，过期后降低优先级。null = 永不过期
  例：面试在4月7日 → expires = "2026-04-08"
    ...
    def _rebuild_profile(self, user_id: str) -> None:
        ...
        for m in memories:
            ...
            if cat == "identity" and key:
                ...
                # Parse "key: value" format from explicit memories
                if ": " in content:
                    _, _, val = content.partition(": ")
                    if key and ("dislike" in key or "不" in content):
    ...
    def _format_memory_context(
        ...
        if episodes:
            ...
            for ep in episodes[:5]:
                ep_lines.append(f"{ep['date']}：{ep['summary']}")
            sections.append("[最近]\n" + "\n".join(ep_lines))

        # Tier 3 / full inject: Relevant memories
        if memories:
            mem_lines = []
            for m in memories:
