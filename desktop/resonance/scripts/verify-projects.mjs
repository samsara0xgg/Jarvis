...
try {
  ...
  release();
  await tile.getByText('求职 5.5 小时', { exact: false }).waitFor();
  check('a finished sort replaces the view', !(await tile.textContent()).includes('没归类'));
  ...
  check('a column hover names its day and time', (await firstCard.locator('.project-day').last().getAttribute('title')).includes('30 分钟'));
