...
function App() {
  const [s, dispatch] = useReducer(reducer, { ...initialState, mode: 'idle' });
        ...
        <div className="control-row" role="toolbar" aria-label="Jarvis 语音控制 · 演示，无真实录音" onContextMenu={e => {
          ...
          <div className="capsule-main-view" hidden={s.mode === 'text'}>
            <PresentationCapsule presentation={s.mode === 'voice' ? 'expanded' : 'collapsed'} active={s.mode !== 'text'} nativeSurface
              ...
              onMicrophoneToggle={() => { feedback(s.micMuted ? 'mic-on' : 'mic-off'); dispatch({ type: 'mic' }); }}
              onSpeakerToggle={() => { feedback(s.soundMuted ? 'speaker-on' : 'speaker-off'); dispatch({ type: 'sound' }); }}
