...
export function PresentationCapsule({ presentation, presence, restState = 'standby', onCollapse, onActivate,
  ...
  color = '#a8b5ff', playbackRate = 1, renderScale = 1, active = true, nativeSurface = false, unreadCount = 0, inboxOpen = false, waveOnly = false }: {
  presentation: CapsulePresentation; presence: Presence; restState?: RestState;
  ...
  return <div ref={root} className={`presentation-capsule ${waveOnly ? 'is-wave-only' : ''} ${collapsed ? 'is-collapsed' : 'is-expanded'}`} data-presentation={waveOnly ? "expanded" : presentation}>
    ...
    <div className={`presentation-core ${nativeSurface ? "glass" : ""}`} data-glass={nativeSurface ? 20 : undefined} data-interactive>
