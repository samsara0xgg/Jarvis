...
- `npm run companion`：刘海小球交互原型（独立进程与资料目录，可与常驻版同时运行，测试时先 `Cmd+Shift+J` 收起常驻版刘海）。小球是星核（玻璃球里有星星，默认皮肤深空玻璃），住在刘海左侧的软件延伸里：靠近探头、悬停出来并显示键盘入口、轻戳开始／打断／结束语音、文字框在她正下方、刘海悬停打开 Dashboard 时她吸附到面板上沿、收起后回家。她只有一个，跟着鼠标换屏：先缩回当前的岛，再从另一块屏的岛里冒出来；没有刘海的屏幕上她住在顶部正中间的黑色胶囊里，悬停胶囊两翼打开 Dashboard。皮肤另有星云、银河、磨砂、极光：按住她到发抖再松手，她原地跳起转一圈、闪一下，换成下一套；菜单栏 ● 的“皮肤”可以直接挑，“自己换装”打开时她每隔 6 到 14 分钟会自己出来试一套、下次再换回你挑的那套，挑的皮肤重启后保留。表情用的是星核表情表里的 17 个加待机：聆听、思考、回应、文字回复完成在演示流程里会自动出现，其余在菜单“看表情”里点一下，她出来演几秒再回家。Dashboard 默认“围着她（一列）”：首页是她的话、Now、Agents（Claude 和 Codex 会话）、Usage 圆环，最后一行插件和项目两个小格；点一行原地长成详情页，面板高度不变，左上角“‹”或 Esc 返回，她按页面换表情；底部横条贴在面板底边，鼠标移上去长成输入框。菜单里可切回原来的两列。转写、回复和 Dashboard 数据全部是本地模拟，不连 daemon、不录音、不播放语音；Agents 的批准、跳转和插件登录只是演示。菜单栏 ● 还可打开 Dashboard 或退出。`npm run build && node scripts/verify-companion.mjs` 在无头 Chrome 里走完九个场景（第七个是换到没有刘海的屏幕，第八个是换皮肤，第九个是 Dashboard 各页），截图在 `evidence/companion/`。
