...
def run(url: str) -> bool:
    ...
    print(f"  extracted text  {len(html_text.encode('utf-8'))} bytes (lossy={lossy})")
    preview = " ".join(html_text.split())[:220]
    ...
    ok = http_ok and bool(html_text.strip())
    if not http_ok:
...
def main() -> int:
    print(f"drain cap = {DEFAULT_WEB_FETCH_MAX_BYTES} bytes (text cap is the dispatcher's)")
